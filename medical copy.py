# st.image("/home/addo/dev/projects/blogs/public/new-2.jpeg", width=800)
import pickle
import numpy as np
import streamlit as st

model = pickle.load(open('model.pkl', 'rb'))

# Page configuration
st.set_page_config(
    page_title="Insurance Charges Prediction",
    page_icon="🏥",
    layout="centered",
)

# Inject CSS to set the background image
page_bg_img = """
<style>
    body {
        background-image: url("data:image/jpeg;base64,");
        background-size: cover;
        background-position: center;
        background-repeat: no-repeat;
        color: #333;
    }

    .stApp {
        background-color: rgba(255, 255, 255, 0);  /* Add transparency to the content */
        border-radius: 10px;
        padding: 10px;
    }

    h1, h2, h4 {
        color: #333;
        text-align: center;
    }
</style>
"""

st.markdown(page_bg_img, unsafe_allow_html=True)

# Page content
st.title("Insurance Charges Predictor")
st.markdown(
    "<h4 style='text-align: center;'>Estimate your medical insurance charges</h4>",
    unsafe_allow_html=True,
)

st.header("Enter Your Information")

# Input fields
age = st.slider("Age", 14, 70)
gender = st.radio("Gender", ["Male", "Female"])

st.subheader("BMI Calculation")
weight_unit = st.radio("Weight unit", ["kg", "lbs"])
weight = st.number_input(f"Weight ({weight_unit})", min_value=1.0, value=70.0)

height_unit = st.radio("Height unit", ["m", "cm", "ft"])
if height_unit == "m":
    height = st.number_input("Height (m)", min_value=0.1, value=1.70)
elif height_unit == "cm":
    height_cm = st.number_input("Height (cm)", min_value=1.0, value=170.0)
    height = height_cm / 100
else:
    height_ft = st.number_input("Height (ft)", min_value=1.0, value=5.5)
    if height_ft < 4.0:
        st.error("Height must be at least 4 feet")
        st.stop()
    elif height_ft >= 9.0:
        st.error("Height must be less than 9 feet")
        st.stop()
    else:
        height = height_ft * 0.3048

if weight_unit == "lbs":
    weight = weight * 0.453592

bmi = weight / (height**2)
st.write(f"Your BMI is: {bmi:.2f}")

children = st.slider("Number of children", 0, 10)
smoker = st.radio("Are you a smoker?", ["No", "Yes"])
region = st.selectbox("Region", ["Northeast", "Southeast", "Northwest", "Southwest"])

# Function to map values
def map_value(value: str, category: str):
    mapping = {
        "gender": {"male": 0, "female": 1},
        "smoker": {"no": 0, "yes": 1},
        "region": {"northeast": 0, "southeast": 1, "northwest": 2, "southwest": 3},
    }
    return mapping[category].get(value.lower())

# Prediction
st.header("Your Estimated Insurance Charges")

if st.button("Calculate Charges"):
    try:
        inp1 = int(age)
        inp2 = int(children)
        inp3 = float(bmi)
        inp4 = map_value(smoker, "smoker")
        inp5 = map_value(region, "region")
        inp6 = map_value(gender, "gender")
        X = [inp1, inp2, inp3, inp4, inp5, inp6]

        charges = model.predict([X])

        st.markdown(
            f"<h2 style='text-align: center;'>Estimated charges: ${int(charges[0]):,}</h2>",
            unsafe_allow_html=True,
        )

    except Exception as e:
        st.error(f"Error in prediction: {str(e)}")

st.markdown("---")
st.subheader("Understanding Your Insurance Charges")
st.write(
    """
The estimated insurance charges are based on several factors including your age, BMI, number of children, smoking status, region, and gender. Here's how these factors generally affect your insurance charges:

- **Age**: Typically, older individuals may have higher insurance charges.
- **BMI**: A higher BMI often correlates with higher insurance charges.
- **Children**: The number of children can affect your insurance charges.
- **Smoking**: Smokers generally face higher insurance charges.
- **Region**: Insurance charges can vary by region due to differences in healthcare costs.
- **Gender**: In some cases, gender may influence insurance charges.

Remember, this is an estimation based on a predictive model and may not reflect actual charges from insurance providers.
"""
)
